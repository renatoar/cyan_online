	...
	public void genJava(PWInterface pw, Env env) {
		...
		if ( isAbstract ) {
			...
			if ( expr != null ) {
				...
				// statement.
				if ( (returnType == Type.Nil && !isInit1 && !statementList.getAlwaysReturnMemoize()) && !hasJavaCode ) {
					pw.printlnIdent("return " +
			...
			if ( expr != null ) {
				if ( this.isInitMethod() || this.isInitOnce() ) {
					env.error(this.getFirstSymbol(),
							"'init', 'init:', and 'initOnce' methods cannot be set with an expression");
				}
					...
					for (final Statement s : this.statementList.getStatementList()) {
						if ( s instanceof ast.StatementAssignmentList ) {
							...
							if ( exprList.size() != 2 ) {
								...
								// throwException)
								env.error(s.getFirstSymbol(),
										"Only one '=' symbol is allowed per statement in an 'initOnce' method", true,
										false);
							}
							for (int j = 0; j < exprList.size() - 1; ++j) {
								final Expr anExpr = exprList.get(j);
								if ( anExpr instanceof ExprIdentStar ) {
									...
									if ( id.getIdentStarKind() == IdentStarKind.instance_variable_t ) {
										...
										if ( !iv.isShared() ) {
											env.error(s.getFirstSymbol(),
													"Non-shared fields cannot be initialized in 'initOnce' methods",
													true, false);
										}
										wasInitializedSet.add(iv);
									}
									...
									if ( !iv.isShared() ) {
										env.error(s.getFirstSymbol(),
												"Non-shared fields cannot be initialized in 'initOnce' methods", true,
												false);
									}
									wasInitializedSet.add(iv);
								}
							...
							// not an assignment
							env.error(s.getFirstSymbol(), "Only assignments are allowed inside method 'initOnce'", true,
									false);
						}
					}
					for (final FieldDec v : currentObject.getFieldList()) {
						if ( v.isShared() && v.getExpr() == null && !wasInitializedSet.contains(v) ) {
							env.error(getFirstSymbol(), "Shared field '" +
									v.getName() +
									"' is not being initialized in the 'initOnce' method", true, false);
						}
					}
