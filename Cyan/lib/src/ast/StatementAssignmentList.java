...
public class StatementAssignmentList extends Statement {
	...
	public void genJava(PWInterface pw, Env env) {
		...
		if ( env.getAddTypeInfo() &&  leftType == Type.Dyn  &&
			...
			addParameterTypeInfo(pw, env, leftSide);
		}
	...
	private void addParameterTypeInfo(PWInterface pw, Env env,
		...
		if ( e.getIdentSymbolArray().size() == 1 && e.getVarDeclaration() != null ) {
			...
			if ( vdi.getTypeInDec() == null ) {
				// type was not supplied
				offsetToInsertType = vdi.getFirstSymbol().getOffset();
			}
			else {
				offsetToInsertType = vdi.getTypeInDec().getFirstSymbol().getOffset();
			}
			...
			if ( vdi.getTypeInDec() != null ) {
				first = vdi.getTypeInDec().getFirstSymbol();
				wasTypeSupplied = true;
			}
			else {
				first = vdi.getFirstSymbol();
			...
			pw.printlnIdent("cyan.lang._System.writeToFileToAddTypeInfo( " +
					...
					"\\r\\n\" + \r\n"
					+ "        \"      \\\"method\\\": \\\"" + env.getCurrentMethod().getNameWithParamAndTypes() + "\\\",\\r\\n\" + \r\n"
			...
			pw.printlnIdent(
					"cyan.lang._System.writeToFileToAddTypeInfo( " +
							...
							offsetToInsertType + ",");
			pw.printlnIdent(
					vdi.getJavaName() + " instanceof cyan.lang._Nil ? \"Nil\" : (" +
							"((cyan.lang._Any ) " + vdi.getJavaName() + ")._prototypePackageName().s + \".\" +");
			pw.printlnIdent(  "((cyan.lang._Any ) " + vdi.getJavaName() + ")._prototypeName().s ),");
			pw.printlnIdent(" " + wasTypeSupplied + ", ");
